    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="max-w-6xl mx-auto p-4 md:p-8 bg-white shadow-2xl rounded-xl border border-slate-200 my-4">
            ...
            <div className="border-[1.5px] border-black w-full flex mb-12 bg-white overflow-hidden rounded-sm">
                <div className="w-1/3 border-r-[1.5px] border-black p-8 flex items-center justify-center bg-slate-50">
                    {/* <span className="text-[10px] font-black text-slate-300 tracking-[0.3em] uppercase">Logo Empresa</span> */}
                </div>

                <div className="w-1/3 border-r-[1.5px] border-black flex flex-col justify-center p-0">
                    <label className="text-[9px] font-black text-slate-400 px-2  uppercase">Título del Formato</label>
                    <textarea
                        {...register("nombreFormato")}
                        rows={2}
                        className="w-full text-center font-black uppercase text-sm outline-none focus:bg-yellow-50 px-4 pb-4 resize-none"
                        placeholder="NOMBRE DEL FORMATO"
                    />
                </div>

                <div className="w-1/6 border-r-[1.5px] border-black flex flex-col text-[10px] font-bold bg-white">

                    <span className="text-[9px] font-black text-slate-400 mb-1"  >version</span>
                    <input {...register("version")} className="border-b-[1.5px] border-black p-2 text-center outline-none focus:bg-yellow-50" placeholder="VERSIÓN" />
                    <span className="text-[9px] font-black text-slate-400 mb-1" >fe creacion</span>
                    <input {...register("fechaCreacion")} className="border-b-[1.5px] border-black p-2 text-center outline-none focus:bg-yellow-50" placeholder="CREACIÓN" />
                    <span className="text-[9px] font-black text-slate-400 mb-1" >Actualizacio</span>
                    <input {...register("fechaActualizacion")} className="border-b-[1.5px] border-black p-2 text-center outline-none focus:bg-yellow-50" placeholder="ACTUALIZACIÓN" />
                    <span className="text-[9px] font-black text-slate-400 mb-1" >Revision</span>
                    <input {...register("fechaRevision")} className="p-2 text-center outline-none focus:bg-yellow-50" placeholder="REVISIÓN" />
                </div>

                <div className="w-1/6 flex flex-col p-2 justify-center items-center bg-slate-50">
                    <span className="text-[9px] font-black text-slate-400 mb-1">SIG UBICACIÓN</span>
                    <input
                        {...register("numeralSIG")}
                        className="w-full text-center text-xs font-black bg-transparent outline-none border-b border-black border-dashed"
                        placeholder="CÓDIGO"
                    />
                </div>
            </div>
